...
- **AgingTrajectoryChart**: A diverging bar chart showing weekly aging velocity relative to the standard aging baseline. Each bar represents `bioAge − standardAging` for that week: teal bars point downward (aging slower = good), rose bars point upward (aging faster = bad). Gradient fills, subtle glow, and a dashed reference line at zero make the chart premium and readable. Accepts `data: TrajectoryPoint[]` (`{ week, bioAge, baseline }`). Computes the overall delta automatically. Used on the home screen.
