...
import { Dumbbell, FileText, Home, MessageSquare, QrCode } from "lucide-react";
...
function NavItem({
	...
	label,
	isAlert,
}: (typeof NAV_ITEMS)[number] & { isAlert: boolean }) {
	return (
		<Link
			to={to}
			className={cn(
			...
			activeProps={{
				className: cn(
					"flex flex-col items-center justify-center px-2 py-1 rounded-2xl transition-all duration-300",
